...
export default async function afterPack(context) {
  ...
  // Windows, macOS, and Linux x64 ship a bundled Node runtime. Linux without a
  // bundled runtime is allowed to fall back to a system Node, but a build that
  // *meant* to bundle Node but lost the binary along the way would silently run
  // the kernel under Electron-as-Node — which is the configuration this project
  // exists to avoid.
  const platformName = context.packager.platform.name
  if (platformName === 'mac' || platformName === 'linux') {
    const nodePath = join(destination, 'node')
